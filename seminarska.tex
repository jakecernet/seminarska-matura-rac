...
\title{Aplikacija za sledenje javnemu prometu}
...
\begin{center}
    ...
    \Huge{\textbf{Aplikacija za sledenje javnemu prometu}}

    \normalsize
    \vspace{\fill}

    Mentor: Matjaž Majnik, univ. dipl. inž. \hfill Avtor: Jaka Černetič, G 4. A\\
    \null
...
Mednarodni standard za izmenjavo podatkov o javnem potniškem prometu je GTFS (General Transit Feed Specification), ki ga je prvotno razvil Google v sodelovanju s Portlandsko metropolitansko agencijo za transport \cite{gtfs}. Standard GTFS definira statične podatke (vozni redi, postajališča, linije), medtem ko razširitev GTFS-Realtime omogoča prenos dinamičnih podatkov (zamude, pozicije vozil, opozorila). Čeprav slovensko potniško omrežje v večji meri sledi tem standardom, posamezni sistemi implementirajo lastne razširitve in nestandardne vmesnike.

\subsection{Obstoječe rešitve in njihove pomanjkljivosti}

Na slovenskem trgu obstaja več delnih rešitev. Uradna aplikacija LPP Ljubljana \cite{lppapp} in zunanja aplikacija Travana pokrivata izključno mestne avtobuse v Ljubljani. Spletni portal BrezAvta \cite{ijppweb} nudi načrtovanje poti od točke do točke, ni pa primeren za spremljanje trenutnih pozicij avtobusov. Portal SŽ \cite{szweb} omogoča vpogled v red vožnje, ne prikazuje pa položajev vlakov na zemljevidu.
...
Ločevanje odjemalske in strežniške plasti zagotavlja, da je vsa logika prikaza in obdelave podatkov na strani odjemalca, medtem ko strežniška plast služi zgolj kot posrednik pri zahtevah, ki jih brskalniki sicer blokirajo zaradi varnostnih politik CORS. Statični podatki, kot so seznami postaj in poti linij, so shranjeni na platformi GitHub v obliki JSON datotek, ki jih aplikacija pridobi neposredno brez proxy posrednika, saj ta endpoint dovoljuje CORS dostop.

\subsection{Odjemalska stran - React SPA}

Odjemalska aplikacija je organizirana v hierarhijo komponent, pri čemer je korenski element komponenta \texttt{App}, ki upravlja globalno stanje in koordinira izmenjavo podatkov med zavihki. Navigacijo med štirimi glavnimi razdelki (zemljevid, postaje, linije, nastavitve) zagotavlja knjižnica React Router z načinom hash routinga, ki je primeren za statično gostovanje na platformah brez podpore za strežniško usmerjanje.

Komponente zavihkov so naložene z leno inicializacijo (\texttt{React.lazy}), kar pomeni, da se koda posameznega zavihka prenese šele ob prvem obisku tega zavihka, s čimer se skrajša čas do prve interaktivnosti aplikacije.

\subsection{Strežniška plast -- brezstrežniške funkcije (serverless functions)}

Mapa \texttt{api/} v repozitoriju s kodo vsebuje šest JavaScript datotek, ki se ob namestitvi na Vercel samodejno pretvorijo v HTTP-končne točke. Vsaka datoteka sledi vzorcu CORS-posrednika:
...
Trajnost stanja med sejami je zagotovljena s shranjevanjem v lokalno shrambo (\texttt{localStorage}) brskalnika, kjer se ob vsaki posodobitvi zapišejo nastavitve teme, aktivno postajališče, nastavitve na zemljevidu prikazanih slojev in seznam priljubljenih postajališč.

\section{Podatkovni sloj - modul \texttt{Api.jsx}}

\subsection{Pregled modula}

Datoteka \texttt{src/Api.jsx} je osrednji podatkovni sloj aplikacije in obsega okoli 700 vrstic kode. Vsebuje vse funkcije za pridobivanje podatkov iz zunanjih virov, logiko za predpomnjenje, pomožne funkcije za formatiranje časovnih podatkov ter algoritme za dekodiranje geografskih poti in interpolacijo položajev vozil.
...
Sledenje vlakom je tehnično zahtevnejše kot sledenje avtobusom, saj SŽ API ne oddaja dejanskih položajev vlakov s frekvenco, primerno za gladko gibanje na karti. Rešitev temelji na predhodnem računanju celotne poti vlaka s časovnimi žigi in naknadnim interpoliranju položaja na osnovi sistemskega časa.
...
// Izracunaj razdalje med tockami
let totalDist = 0;
const dists = [0];
...
Osnova za kartografski prikaz je knjižnica MapLibre GL JS \cite{maplibre}, odprtokodna nadgradnja projekta Mapbox GL JS. Knjižnica za upodabljanje zemljevida uporablja tehnologijo WebGL, kar zagotavlja gladko gibanje in visoko zmogljivost tudi pri prikazu tisočih točk hkrati. Zemljevida sta dva: svetel (OpenStreetMap Raster Light) in temen (OpenStreetMap Raster Dark), med njima pa lahko preklopi uporabnik. Na sliki 2 je prikazan svetli zemljevid z izrisanimi položaji avtobusov in postajališč.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{img/map.png}
\caption{Zavihek z zemljevidom}
\end{figure}
...
Vsaka entiteta na karti je urejena v plast (\textit{layer}), ki je vezana na vir podatkov (\textit{source}) tipa GeoJSON. Avtobusi, avtobusna postajališča, vlaki in železniške postaje so ločeni sloji, kar omogoča neodvisno vklapljanje in izklapljanje posameznih slojev zemljevida.

\subsection{Gruče postajališč in avtobusov}

Postajališča in avtobusi so pri nižji stopnji povečave prikazani v obliki gruč (cluster). Ko je elementov preveč za pregleden prikaz, se samodejno združijo v krog s številko, ki pove, koliko jih je v skupini. Z večanjem povečave se gruče razpustijo v posamezne postaje in avtobuse.

\subsection{Prikaz izbrane poti}

Ob izbiri vozila iz kateregakoli zavihka se na karti izriše celotna trasa linije s postajišči. Trasa je prikazana kot linijski sloj na vrhu vseh ostalih slojev, postajišča na trasi pa so označena s krogi, ki so barvno usklajeni z barvno shemo prevoznika. Komponenta \texttt{RouteTab} prikaže predal s podrobnostmi o vsakem postajališču, vključno s predvidenimi časi prihodov, kot je prikazano na sliki 3.

\begin{figure}[tb]
\centering
\includegraphics[width=0.9\textwidth]{img/routa.png}
\caption{Prikaz trase izbrane linije na karti}
\label{fig:map}
\end{figure}
...
Zavihek \texttt{StationsTab} omogoča iskanje postajališč po imenu in prikaz najbližjih glede na lokacijo uporabnika. Implementiran je v treh pogledih: \textit{V bližini} (privzeto), \textit{Vsa postajališča} in \textit{Priljubljena}. Primer seznama postajališč je prikazan na sliki 4.
...
Priljubljena postajališča so shranjena v \texttt{localStorage} in so dostopna tudi brez omrežne povezave. Razdalja od uporabnika do postaje je izračunana z Haversine formulo in prikazana poleg vsakega postajališča.

\begin{figure}[tb]
\centering
\includegraphics[width=0.9\textwidth]{img/postaje.png}
\caption{Seznam postaj}
\label{fig:map}
\end{figure}

\subsection{Zavihek Linije}

Zavihek \texttt{LinesTab} prikazuje prihode za izbrano postajališče, ločene po virih (LPP, IJPP, SŽ), ter seznam vseh aktivnih linij, pridobljenih iz GPS podatkov. Omogoča iskanje linij po imenu in označevanje priljubljenih linij.

Ob kliku na prihod ali linijo se izvede funkcija \texttt{getTripFromId}, ki pridobi podrobnosti o vožnji in jih posreduje zemljevidu za prikaz trase. Prihodi so prikazani z ETA vrednostjo in absolutnim časom prihoda v formatu HH:mm, kot je prikazano na sliki 5.

\begin{figure}[tb]
\centering
\includegraphics[width=0.9\textwidth]{img/linije.png}
\caption{Prikaz prihodov linij na postajo}
\label{fig:map}
\end{figure}
...
Eden najpomembnejših tehničnih izzivov je bilo dejstvo, da LPP in BrezAvta API-ja za identifikacijo postajališč ne uporabljata istih identifikatorjev. LPP ima lasten sistem šifer postajališč (\texttt{ref\_id}, \texttt{station\_code}), BrezAvta pa operira z identifikatorji, ki so skladni z GTFS standardom (\texttt{gtfs\_id}).
...
LPP API zahteva, da ga naslavljamo prek Vercel proxy-ja, saj neposredni dostop iz brskalnika ni dovoljen. Podatke vrača v obliki \texttt{\{data: \{arrivals: [...]\}\}}, kjer so prihodi urejeni po \texttt{eta\_min}. API je razmeroma zanesljiv in dokumentiran.
...
Rešitev je bila implementacija ločenih funkcij, ki formatirajo izhod v enotno interno obliko.
...
\begin{lstlisting}[caption={Samozaznavno dekodiranje Polyline-a z nastavljivo natančnostjo}, label={lst:poly}]
...
\subsection{Nalaganje aplikacije na GitHub Pages in Vercel}

Nameščanje aplikacije je avtomatizirano s skriptami v \texttt{package.json}: ukaz \texttt{npm run build} zgradi optimizirano različico, ukaz \texttt{npm run deploy} pa jo objavi na GitHub Pages prek paketa \texttt{gh-pages}. Vercel brezstrežniške funkcije se samodejno objavijo ob vsaki potisni objavi (push) v repozitorij.

\section{Zaključek}

Seminarska naloga predstavlja razvoj aplikacije IJPP Tracker, ki uspešno združuje podatke treh ločenih sistemov javnega potniškega prometa v Republiki Sloveniji v enoten spletni vmesnik s sprotnim sledenjem vozil. Med razvojem je bilo ugotovljeno, da dostopnost in stabilnost javnih API-jev ostajata izziv: API IJPP vzdržuje ekipa prostovoljcev, prav tako API SŽ, kar pomeni, da lahko pride do občasnih nedostopnosti ali sprememb v strukturi podatkov, ki zahtevajo prilagoditve v aplikaciji. 
...
\subsection{Izvorna koda aplikacije}

Celotna izvorna koda aplikacije IJPP Tracker je dostopna v javnem repozitoriju na platformi GitHub:

\begin{itemize}
\item \textbf{Odjemalska aplikacija (React):} \url{https://github.com/jakecernet/ijpp-tracker}
\item \textbf{Serverless funkcije (Vercel):} Vključene v istem repozitoriju v mapi \texttt{api/}
\item \textbf{Seznami postajališč:} \url{https://github.com/jakecernet/ijpp-json}
\item \textbf{Spletna stran:} \url{https://tracker.cernetic.cc}
\end{itemize}
...
\begin{samepage}
    ...
    Izjavljam, da je seminarska naloga z naslovom \textit{Aplikacija za sledenje javnemu prometu} v celoti moje avtorsko delo, ki sem ga izdelal samostojno s pomočjo navedene literature in pod vodstvom mentorja.
